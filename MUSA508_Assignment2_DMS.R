...
#Adjusting for inflaction
tracts09$MedHHInc <- tracts09$MedHHInc*1.42
tracts09$MedRent <- tracts09$MedRent*1.42
...
ggplot() + 
  ...
  labs(title="2009 Light Rail Stops", 
...
ggplot() + 
  ...
  geom_sf(data=LightRailPGH_sf, 
          aes(colour = Direction), 
          show.legend = "point", size= 3) +
  labs(title="2017 Light Rail Stops", 
       subtitle="Pittsburgh, PA", 
       caption="Figure 1") +
  mapTheme()
...
#  mutate(MedHHInc.inf = ifelse(year == "2009", MedHHInc * 1.42, MedHHInc))
...
ggplot(allTracts.group)+
  ...
  labs(title = "Population Percent White 2009-2017", subtitle = "") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
...
ggplot(allTracts.group)+
  ...
  scale_fill_manual(values = palette5,
                    ...
                    name = "Median Age\n(Quintile Breaks)") +
  labs(title = "Median Age 2009-2017", subtitle = "") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
...
clip_all$year <- as.character(clip_all$year)

#Plot version 1
p <- ggplot(clip_all, aes(y=avgRent, x=distance))
p + geom_line(aes(group=year, color=year), size = 1.5) + 
  geom_point(aes(color=year), size = 3) +
  ggtitle("Rent as a function of Distance to Station") +
  plotTheme() +
  theme() 
...
clip_crime <- 
  ...
  dplyr::select(distance, INCIDENTHIERARCHYDESC) %>%
  mutate(Selection_Type = "Clip") %>%
  group_by(distance) %>% 
  summarise(Total_Count = n())

ggplot(data = clip_crime, aes(x=distance, y=Total_Count)) +
  geom_line(size = 1.5) +
  geom_point(size = 3) +
  ggtitle("Instances of Thefts as a Function of Distance to Station (last 30 days)") +
  xlab("Distance to Station (ft)") +
